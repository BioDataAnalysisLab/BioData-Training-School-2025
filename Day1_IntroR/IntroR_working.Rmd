---
  title: "Intro to R and Data Handling"
...
We are going to use example data from Software Carpentry. I have already downloaded it and included it in the directory where this code is. If the directory with this code is showing in the Files pane in the bottom right, you can click on the More and select "Copy Folder Path to Clipboard." Paste that below where it says "PASTE HERE" and remove the hashtag.
...
Let's start with `filter()` and `select()`
```{r}
